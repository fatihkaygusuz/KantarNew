import svgPaths from "./svg-eftfdb6hto";
import image2 from "../image-2.png";

function ArrowNarrowRight({ className }: { className?: string }) {
  return (
    <div className={className || "overflow-clip relative size-[24px] d1:size-[18.24px] d2:size-[19.68px] d3:size-[21.12px] d4:size-[22.56px] d5:size-[24px]"} data-name="arrow-narrow-right">
      <div className="absolute bottom-1/4 left-[16.67%] right-[16.67%] top-1/4" data-name="Icon">
        <div className="absolute inset-[-8.33%_-6.25%]">
          <svg className="block size-full" fill="none" height="14" preserveAspectRatio="none" viewBox="0 0 18 14" width="18">
            <path d="M1 7H17M11 13L17 7L11 1" id="Icon" stroke="#1E1E1E" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function EyebrowBlock() {
  return (
    <div className="content-stretch flex flex-col gap-[16px] d1:gap-[12.16px] d2:gap-[13.12px] d3:gap-[14.08px] d4:gap-[15.04px] d5:gap-[16px] items-start relative shrink-0 w-full" data-name="Eyebrow Block">
      <p className="[word-break:break-word] font-['Inter:Bold',sans-serif] font-bold leading-[32px] d1:leading-[24.32px] d2:leading-[26.24px] d3:leading-[28.16px] d4:leading-[30.08px] d5:leading-[32px] min-w-full not-italic relative shrink-0 text-[#5f9cf8] text-[24px] d1:text-[18.24px] d2:text-[19.68px] d3:text-[21.12px] d4:text-[22.56px] d5:text-[24px] w-[min-content]">INTELLIGENCE FOR BRAND GROWTH</p>
      <div className="h-0 relative shrink-0 w-[56px] d1:w-[42.56px] d2:w-[45.92px] d3:w-[49.28px] d4:w-[52.64px] d5:w-[56px]" data-name="Line">
        <div className="absolute inset-[-3px_0_0_0]">
          <svg className="block size-full" fill="none" height="3" preserveAspectRatio="none" viewBox="0 0 56 3" width="56">
            <line id="Line" stroke="#5F9CF8" strokeWidth="3" x2="56" y1="1.5" y2="1.5" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function HeadlineGroup() {
  return (
    <div className="content-stretch flex flex-col items-start pb-[32px] d1:pb-[24.32px] d2:pb-[26.24px] d3:pb-[28.16px] d4:pb-[30.08px] d5:pb-[32px] relative shrink-0 w-full" data-name="HeadlineGroup">
      <p className="[word-break:break-word] font-['Inter:Bold',sans-serif] font-bold leading-[1.25] not-italic relative shrink-0 text-[#1e1e1e] text-[32px] md:text-[48px] d1:text-[54.72px] d2:text-[59.04px] d3:text-[63.36px] d4:text-[67.68px] d5:text-[72px] tracking-[-1.44px] d1:tracking-[-1.09px] d2:tracking-[-1.18px] d3:tracking-[-1.27px] d4:tracking-[-1.35px] d5:tracking-[-1.44px] w-full">{`Know what's changing and what's to do next.`}</p>
    </div>
  );
}

function Frame1() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-full">
      <HeadlineGroup />
      <p className="[word-break:break-word] font-['Inter:Regular',sans-serif] font-normal leading-[30px] d1:leading-[22.8px] d2:leading-[24.6px] d3:leading-[26.4px] d4:leading-[28.2px] d5:leading-[30px] not-italic relative shrink-0 text-[#616161] text-[16px] d1:text-[12.16px] d2:text-[13.12px] d3:text-[14.08px] d4:text-[15.04px] d5:text-[16px] md:text-[20px]">Sharper understanding. Smarter decisions. Stronger growth</p>
    </div>
  );
}

function ArrowRight() {
  return (
    <div className="relative shrink-0 size-[24px] d1:size-[18.24px] d2:size-[19.68px] d3:size-[21.12px] d4:size-[22.56px] d5:size-[24px]" data-name="arrow-right">
      <svg className="absolute block inset-0 size-full" fill="none" height="24" preserveAspectRatio="none" viewBox="0 0 24 24" width="24">
        <g id="arrow-right">
          <path d="M5 12H19M12 19L19 12L12 5" id="Vector" stroke="#CDA3F6" strokeLinecap="round" strokeWidth="2" />
        </g>
      </svg>
    </div>
  );
}

function ExploreCta() {
  return (
    <div className="content-stretch flex flex-col gap-[14px] d1:gap-[10.64px] d2:gap-[11.48px] d3:gap-[12.32px] d4:gap-[13.16px] d5:gap-[14px] items-start relative shrink-0" data-name="ExploreCTA">
      <div className="content-stretch flex items-center justify-between relative shrink-0 w-full" data-name="Frame">
        <p className="[word-break:break-word] font-['Inter:Medium',sans-serif] font-medium leading-[24px] d1:leading-[18.24px] d2:leading-[19.68px] d3:leading-[21.12px] d4:leading-[22.56px] d5:leading-[24px] not-italic relative shrink-0 text-[#1e1e1e] text-[16px] d1:text-[12.16px] d2:text-[13.12px] d3:text-[14.08px] d4:text-[15.04px] d5:text-[16px] whitespace-nowrap">Discover Our Solutions</p>
        <ArrowRight />
      </div>
      <div className="h-0 relative shrink-0 w-[200px] d1:w-[152px] d2:w-[164px] d3:w-[176px] d4:w-[188px] d5:w-[200px]" data-name="CTARule">
        <div className="absolute inset-[-1px_0_0_0]">
          <svg className="block size-full" fill="none" height="1" preserveAspectRatio="none" viewBox="0 0 200 1" width="200">
            <line id="CTARule" stroke="#C2C2C2" x2="200" y1="0.5" y2="0.5" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function NavigationControls() {
  return (
    <div className="content-stretch flex gap-[16px] d1:gap-[12.16px] d2:gap-[13.12px] d3:gap-[14.08px] d4:gap-[15.04px] d5:gap-[16px] items-center overflow-clip relative shrink-0" data-name="NavigationControls">
      <p className="[word-break:break-word] font-['Inter:Regular',sans-serif] font-normal leading-[normal] not-italic relative shrink-0 text-[#1e1e1e] text-[28px] d1:text-[21.28px] d2:text-[22.96px] d3:text-[24.64px] d4:text-[26.32px] d5:text-[28px] whitespace-nowrap">‹</p>
      <div className="flex h-[0.01px] d1:h-[0.01px] d2:h-[0.01px] d3:h-[0.01px] d4:h-[0.01px] d5:h-[0.01px] items-center justify-center relative shrink-0 w-[20px] d1:w-[15.2px] d2:w-[16.4px] d3:w-[17.6px] d4:w-[18.8px] d5:w-[20px]">
        <div className="flex-none rotate-90">
          <div className="h-[20px] d1:h-[15.2px] d2:h-[16.4px] d3:h-[17.6px] d4:h-[18.8px] d5:h-[20px] relative w-[0.01px] d1:w-[0.01px] d2:w-[0.01px] d3:w-[0.01px] d4:w-[0.01px] d5:w-[0.01px]" data-name="NavDivider">
            <div className="absolute bottom-full left-0 right-0 top-[-5%]">
              <svg className="block size-full" fill="none" height="1" preserveAspectRatio="none" viewBox="0 0 0.01 1" width="0.01">
                <line id="NavDivider" stroke="#C2C2C2" x2="0.01" y1="0.5" y2="20.5" />
              </svg>
            </div>
          </div>
        </div>
      </div>
      <p className="[word-break:break-word] font-['Inter:Regular',sans-serif] font-normal leading-[normal] not-italic relative shrink-0 text-[#1e1e1e] text-[28px] d1:text-[21.28px] d2:text-[22.96px] d3:text-[24.64px] d4:text-[26.32px] d5:text-[28px] whitespace-nowrap">›</p>
    </div>
  );
}

function LeftZone() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col gap-[40px] d1:gap-[30.4px] d2:gap-[32.8px] d3:gap-[35.2px] d4:gap-[37.6px] d5:gap-[40px] md:gap-[60px] h-full items-start max-w-full md:max-w-[560px] min-w-0 md:min-w-[400px] relative" data-name="LeftZone">
      <EyebrowBlock />
      <Frame1 />
      <ExploreCta />
      <NavigationControls />
    </div>
  );
}

function MaskGroup() {
  return (
    <div className="absolute h-[817.69px] d1:h-[621.4px] d2:h-[670.5px] d3:h-[719.6px] d4:h-[768.6px] d5:h-[817.7px] left-[842px] d1:left-[639.9px] d2:left-[690.4px] d3:left-[741px] d4:left-[791.5px] d5:left-[842px] top-[80px] d1:top-[60.8px] d2:top-[65.6px] d3:top-[70.4px] d4:top-[75.2px] d5:top-[80px] w-[1083px] d1:w-[823.1px] d2:w-[888.1px] d3:w-[953px] d4:w-[1018px] d5:w-[1083px]" data-name="Mask group">
      <svg className="absolute block inset-0 size-full" fill="none" height="817.69" preserveAspectRatio="none" viewBox="0 0 1083 817.69" width="1083">
        <g id="Mask group">
          <mask height="818" id="mask0_gr2" maskUnits="userSpaceOnUse" style={{ maskType: "alpha" }} width="1083" x="0" y="0">
            <path d={svgPaths.p1a36c200} fill="#D9D9D9" id="Polygon 1" />
          </mask>
          <g mask="url(#mask0_gr2)">
            <g id="Group 4">
              <rect fill="url(#paint0_linear_gr2)" height="834.921" id="Rectangle 1" width="233.051" x="849.951" />
              <rect fill="url(#paint1_linear_gr2)" height="180.142" id="Rectangle 2" width="152.511" x="697.438" y="654.778" />
              <path d={svgPaths.p3f8a5700} fill="url(#paint2_linear_gr2)" id="Rectangle 4" />
              <rect fill="url(#paint3_linear_gr2)" height="366.551" id="Rectangle 5" width="495.233" x="-34.2732" y="468.37" />
              <rect fill="url(#paint4_linear_gr2)" height="245.934" id="Rectangle 3" width="83.9668" x="613.471" y="588.987" />
            </g>
          </g>
        </g>
        <defs>
          <linearGradient gradientUnits="userSpaceOnUse" id="paint0_linear_gr2" x1="966.477" x2="966.477" y1="0" y2="834.921">
            <stop stopColor="#2484FF" stopOpacity="0.8" />
            <stop offset="0.49" stopColor="#A973F9" stopOpacity="0.8" />
            <stop offset="1" stopColor="#FF852D" stopOpacity="0.8" />
          </linearGradient>
          <linearGradient gradientUnits="userSpaceOnUse" id="paint1_linear_gr2" x1="773.693" x2="773.693" y1="654.778" y2="834.921">
            <stop stopColor="#2484FF" stopOpacity="0.4" />
            <stop offset="1" stopColor="#A973F9" stopOpacity="0.4" />
          </linearGradient>
          <linearGradient gradientUnits="userSpaceOnUse" id="paint2_linear_gr2" x1="537.217" x2="537.217" y1="416.677" y2="834.921">
            <stop stopColor="#2484FF" stopOpacity="0.4" />
            <stop offset="1" stopColor="#A973F9" stopOpacity="0.4" />
          </linearGradient>
          <linearGradient gradientUnits="userSpaceOnUse" id="paint3_linear_gr2" x1="213.343" x2="213.343" y1="468.37" y2="834.921">
            <stop stopColor="#2484FF" stopOpacity="0.2" />
            <stop offset="1" stopColor="#A973F9" stopOpacity="0.2" />
          </linearGradient>
          <linearGradient gradientUnits="userSpaceOnUse" id="paint4_linear_gr2" x1="655.454" x2="655.454" y1="588.987" y2="834.921">
            <stop stopColor="#2484FF" />
            <stop offset="1" stopColor="#A973F9" />
          </linearGradient>
        </defs>
      </svg>
    </div>
  );
}

function News() {
  return (
    <div className="bg-white content-stretch flex flex-[1_0_0] flex-col items-start justify-between min-w-full sm:min-w-[280px] overflow-clip pb-[36px] d1:pb-[27.36px] d2:pb-[29.52px] d3:pb-[31.68px] d4:pb-[33.84px] d5:pb-[36px] sm:pb-[48px] pt-[24px] d1:pt-[18.24px] d2:pt-[19.68px] d3:pt-[21.12px] d4:pt-[22.56px] d5:pt-[24px] sm:pt-[32px] px-[24px] d1:px-[18.24px] d2:px-[19.68px] d3:px-[21.12px] d4:px-[22.56px] d5:px-[24px] sm:px-[36px] relative self-stretch" data-name="NEWS">
      <div className="content-stretch flex items-center overflow-clip relative shrink-0">
        <p className="[word-break:break-word] font-['Inter:Semi_Bold',sans-serif] font-semibold leading-[24px] d1:leading-[18.24px] d2:leading-[19.68px] d3:leading-[21.12px] d4:leading-[22.56px] d5:leading-[24px] not-italic relative shrink-0 text-[#8455b0] text-[16px] d1:text-[12.16px] d2:text-[13.12px] d3:text-[14.08px] d4:text-[15.04px] d5:text-[16px] whitespace-nowrap">EVENT</p>
      </div>
      <div className="content-stretch flex gap-[60px] d1:gap-[45.6px] d2:gap-[49.2px] d3:gap-[52.8px] d4:gap-[56.4px] d5:gap-[60px] items-end relative shrink-0 w-full">
        <p className="[word-break:break-word] flex-[1_0_0] font-['Inter:Bold',sans-serif] font-bold leading-[32px] d1:leading-[24.32px] d2:leading-[26.24px] d3:leading-[28.16px] d4:leading-[30.08px] d5:leading-[32px] min-w-px not-italic relative text-[#1e1e1e] text-[24px] d1:text-[18.24px] d2:text-[19.68px] d3:text-[21.12px] d4:text-[22.56px] d5:text-[24px]">Modernising Energy Efficiency through Digitalisation</p>
        <ArrowNarrowRight className="overflow-clip relative shrink-0 size-[24px] d1:size-[18.24px] d2:size-[19.68px] d3:size-[21.12px] d4:size-[22.56px] d5:size-[24px]" />
      </div>
      <div className="absolute bg-[#e0e0e0] bottom-0 right-[0.25px] d1:right-[0.19px] d2:right-[0.2px] d3:right-[0.22px] d4:right-[0.23px] d5:right-[0.25px] top-0 w-px" data-name="Divider" />
    </div>
  );
}

function FinalReport() {
  return (
    <div className="bg-white content-stretch flex flex-[1_0_0] flex-col items-start justify-between min-w-full sm:min-w-[280px] overflow-clip pb-[36px] d1:pb-[27.36px] d2:pb-[29.52px] d3:pb-[31.68px] d4:pb-[33.84px] d5:pb-[36px] sm:pb-[48px] pt-[24px] d1:pt-[18.24px] d2:pt-[19.68px] d3:pt-[21.12px] d4:pt-[22.56px] d5:pt-[24px] sm:pt-[32px] px-[24px] d1:px-[18.24px] d2:px-[19.68px] d3:px-[21.12px] d4:px-[22.56px] d5:px-[24px] sm:px-[36px] relative self-stretch" data-name="FINAL REPORT">
      <div className="content-stretch flex items-center overflow-clip relative shrink-0">
        <p className="[word-break:break-word] font-['Inter:Semi_Bold',sans-serif] font-semibold leading-[24px] d1:leading-[18.24px] d2:leading-[19.68px] d3:leading-[21.12px] d4:leading-[22.56px] d5:leading-[24px] not-italic relative shrink-0 text-[#8455b0] text-[16px] d1:text-[12.16px] d2:text-[13.12px] d3:text-[14.08px] d4:text-[15.04px] d5:text-[16px] whitespace-nowrap">NEWS</p>
      </div>
      <div className="content-stretch flex gap-[66px] d1:gap-[50.16px] d2:gap-[54.12px] d3:gap-[58.08px] d4:gap-[62.04px] d5:gap-[66px] items-end relative shrink-0 w-full">
        <p className="[word-break:break-word] flex-[1_0_0] font-['Inter:Bold',sans-serif] font-bold leading-[32px] d1:leading-[24.32px] d2:leading-[26.24px] d3:leading-[28.16px] d4:leading-[30.08px] d5:leading-[32px] min-w-px not-italic relative text-[#1e1e1e] text-[24px] d1:text-[18.24px] d2:text-[19.68px] d3:text-[21.12px] d4:text-[22.56px] d5:text-[24px]">Executive Director makes official visit to India</p>
        <ArrowNarrowRight className="overflow-clip relative shrink-0 size-[24px] d1:size-[18.24px] d2:size-[19.68px] d3:size-[21.12px] d4:size-[22.56px] d5:size-[24px]" />
      </div>
      <div className="absolute bg-[#e0e0e0] bottom-0 right-[0.25px] d1:right-[0.19px] d2:right-[0.2px] d3:right-[0.22px] d4:right-[0.23px] d5:right-[0.25px] top-0 w-px" data-name="Divider" />
    </div>
  );
}

function TechnologyReport() {
  return (
    <div className="bg-white content-stretch flex flex-[1_0_0] flex-col items-start justify-between min-w-full sm:min-w-[280px] overflow-clip pb-[36px] d1:pb-[27.36px] d2:pb-[29.52px] d3:pb-[31.68px] d4:pb-[33.84px] d5:pb-[36px] sm:pb-[48px] pt-[24px] d1:pt-[18.24px] d2:pt-[19.68px] d3:pt-[21.12px] d4:pt-[22.56px] d5:pt-[24px] sm:pt-[32px] px-[24px] d1:px-[18.24px] d2:px-[19.68px] d3:px-[21.12px] d4:px-[22.56px] d5:px-[24px] sm:px-[36px] relative self-stretch" data-name="TECHNOLOGY REPORT">
      <div className="content-stretch flex items-center overflow-clip relative shrink-0">
        <p className="[word-break:break-word] font-['Inter:Semi_Bold',sans-serif] font-semibold leading-[24px] d1:leading-[18.24px] d2:leading-[19.68px] d3:leading-[21.12px] d4:leading-[22.56px] d5:leading-[24px] not-italic relative shrink-0 text-[#8455b0] text-[16px] d1:text-[12.16px] d2:text-[13.12px] d3:text-[14.08px] d4:text-[15.04px] d5:text-[16px] whitespace-nowrap">FINAL REPORT</p>
      </div>
      <div className="content-stretch flex gap-[12px] d1:gap-[9.12px] d2:gap-[9.84px] d3:gap-[10.56px] d4:gap-[11.28px] d5:gap-[12px] items-end relative shrink-0 w-full">
        <p className="[word-break:break-word] flex-[1_0_0] font-['Inter:Bold',sans-serif] font-bold leading-[32px] d1:leading-[24.32px] d2:leading-[26.24px] d3:leading-[28.16px] d4:leading-[30.08px] d5:leading-[32px] min-w-px not-italic relative text-[#1e1e1e] text-[24px] d1:text-[18.24px] d2:text-[19.68px] d3:text-[21.12px] d4:text-[22.56px] d5:text-[24px]">Renewables 2019</p>
        <ArrowNarrowRight className="overflow-clip relative shrink-0 size-[24px] d1:size-[18.24px] d2:size-[19.68px] d3:size-[21.12px] d4:size-[22.56px] d5:size-[24px]" />
      </div>
    </div>
  );
}

function Event() {
  return (
    <div className="bg-white content-stretch flex flex-[1_0_0] flex-col items-start justify-between min-w-[330px] d1:min-w-[250.8px] d2:min-w-[270.6px] d3:min-w-[290.4px] d4:min-w-[310.2px] d5:min-w-[330px] overflow-clip pt-[32px] d1:pt-[24.32px] d2:pt-[26.24px] d3:pt-[28.16px] d4:pt-[30.08px] d5:pt-[32px] px-[36px] d1:px-[27.36px] d2:px-[29.52px] d3:px-[31.68px] d4:px-[33.84px] d5:px-[36px] relative self-stretch" data-name="EVENT">
      <div className="content-stretch flex items-center overflow-clip relative shrink-0">
        <p className="[word-break:break-word] font-['Inter:Semi_Bold',sans-serif] font-semibold leading-[24px] d1:leading-[18.24px] d2:leading-[19.68px] d3:leading-[21.12px] d4:leading-[22.56px] d5:leading-[24px] not-italic relative shrink-0 text-[#ff854f] text-[16px] d1:text-[12.16px] d2:text-[13.12px] d3:text-[14.08px] d4:text-[15.04px] d5:text-[16px] whitespace-nowrap">TECHNOLOGY REPORT</p>
      </div>
      <div className="content-stretch flex flex-[1_0_0] items-end min-h-px pb-[46px] d1:pb-[34.96px] d2:pb-[37.72px] d3:pb-[40.48px] d4:pb-[43.24px] d5:pb-[46px] relative">
        <p className="[word-break:break-word] font-['Inter:Bold',sans-serif] font-bold leading-[32px] d1:leading-[24.32px] d2:leading-[26.24px] d3:leading-[28.16px] d4:leading-[30.08px] d5:leading-[32px] not-italic relative shrink-0 text-[#db4a14] text-[24px] d1:text-[18.24px] d2:text-[19.68px] d3:text-[21.12px] d4:text-[22.56px] d5:text-[24px] w-[289.25px] d1:w-[219.8px] d2:w-[237.2px] d3:w-[254.5px] d4:w-[271.9px] d5:w-[289.2px]">Electric Vehicle City Casebook 2019</p>
      </div>
      <div className="absolute bg-[#e0e0e0] bottom-0 right-[0.25px] d1:right-[0.19px] d2:right-[0.2px] d3:right-[0.22px] d4:right-[0.23px] d5:right-[0.25px] top-0 w-px" data-name="Divider" />
      <div className="h-0 relative shrink-0 w-full" data-name="CTARule">
        <div className="absolute inset-[-2px_0_0_0]">
          <svg className="block size-full" fill="none" height="2" preserveAspectRatio="none" viewBox="0 0 325.25 2" width="325.25">
            <line id="CTARule" stroke="#FF854F" strokeWidth="2" x2="325.25" y1="1" y2="1" />
          </svg>
        </div>
      </div>
    </div>
  );
}

export default function GlobalReachSection() {
  return (
    <div className="w-full">

      {/* Insight spotlight — 1660px centered */}
      <div className="w-full bg-[#fafafa] overflow-hidden">
        <div className="max-w-[1660px] mx-auto">
          <div className="content-stretch flex h-auto md:h-[900px] items-start justify-between overflow-clip pl-4 md:pl-[88px] d1:pl-[133.8px] d2:pl-[144.3px] d3:pl-[154.9px] d4:pl-[165.4px] d5:pl-[176px] pt-[48px] d1:pt-[36.48px] d2:pt-[39.36px] d3:pt-[42.24px] d4:pt-[45.12px] d5:pt-[48px] md:pt-[80px] pb-[48px] d1:pb-[36.48px] d2:pb-[39.36px] d3:pb-[42.24px] d4:pb-[45.12px] d5:pb-[48px] md:pb-0 relative shrink-0 w-full" data-name="insight-spotlight-section">
            <LeftZone />
            <MaskGroup />
            <p className="-translate-x-full [word-break:break-word] absolute font-['Palanquin:Bold',sans-serif] leading-[normal] left-[1819px] d1:left-[1382.4px] d2:left-[1491.6px] d3:left-[1600.7px] d4:left-[1709.9px] d5:left-[1819px] not-italic text-[#1e1e1e] text-[700px] d1:text-[532px] d2:text-[574px] d3:text-[616px] d4:text-[658px] d5:text-[700px] text-right top-[-229px] d1:top-[-174px] d2:top-[-187.8px] d3:top-[-201.5px] d4:top-[-215.3px] d5:top-[-229px] tracking-[-30px] d1:tracking-[-22.8px] d2:tracking-[-24.6px] d3:tracking-[-26.4px] d4:tracking-[-28.2px] d5:tracking-[-30px] whitespace-nowrap">IQ</p>
          </div>
        </div>
      </div>

      {/* News cards strip — 1660px centered */}
      <div className="w-full bg-white border-t border-[#e0e0e0]">
        <div className="max-w-[1660px] mx-auto">
          <div className="content-stretch flex flex-col sm:flex-row flex-wrap gap-0 h-auto sm:h-[305px] items-start relative shrink-0 w-full" data-name="News Cards Strip">
            <News />
            <FinalReport />
            <TechnologyReport />
            <Event />
          </div>
        </div>
      </div>
    </div>
  );
}
